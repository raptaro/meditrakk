...
interface AppointmentFromAPI {
  ...
  doctor: number;
  doctor_name: string;
  appointment_date: string;      // ISO string, e.g. "2025‑10‑27T10:00:00Z"
  status: string;
...
interface BackendResponsePast {
  past: AppointmentFromAPI[];
  total_count?: number;
  patient_name?: string;
}
  ...
  useEffect(() => {
    async function fetchAppointments() {
      try {
        ...
        const data: BackendResponsePast = await res.json();
        console.log("Backend data (past):", data);

        const allPast = Array.isArray(data.past) ? data.past : [];

        const mapped: AppointmentRow[] = allPast.map(appointment => {
